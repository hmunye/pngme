//! Representation of a PNG (Portable Network Graphics) datastream.
//!
//! A PNG datastream consists of a fixed 8-byte signature followed by a sequence
//! of chunks. Each chunk carries metadata or image data, and the structure of
...
impl PNG {
    /// PNG signature indicating the remainder of the datastream contains a
    ...
    pub fn remove_first_chunk(&mut self, chunk_type: &str) -> Option<Chunk> {
        ...
        {
            // Need to keep the ordering of chunks.
...
impl TryFrom<&[u8]> for PNG {
    ...
    fn try_from(bytes: &[u8]) -> result::Result<Self, Self::Error> {
        ...
        //
        // - 25 bytes: IHDR chunk (13 bytes of data + 12 bytes for length, type,
        // and CRC)
        //
        // - 19 bytes: IDAT chunk (at least 7 bytes of compressed data + 12
        // bytes for length, type, and CRC)
        //
        ...
        // - Compressed data blocks 	            n bytes (at least 1 byte)
        // - Check value 	                        4 bytes
        //
        // - 12 bytes: IEND chunk (no data, just length, type, and CRC)
        let min = 64;
        let len = bytes.len();

        anyhow::ensure!(
            len >= min,
            "invalid PNG datastream: input must be at least 59 bytes, but received: {}",
            len
        );
        ...
        anyhow::ensure!(
            header == Self::MAGIC,
            "invalid PNG datastream: malformed PNG signature: {:?}",
            header
        );
        offset += 8;

        let mut chunks: Vec<Chunk> = Vec::with_capacity(3);

        // 25-byte IHDR chunk.
        let ihdr = Chunk::try_from(&bytes[offset..offset + 25])?;
        anyhow::ensure!(
            ihdr.chunk_type().bytes() == *b"IHDR",
            "invalid PNG datastream: missing IHDR chunk"
        );
        anyhow::ensure!(
            ihdr.data().len() == 13,
            "invalid PNG datastream: malformed IHDR chunk: {}",
            ihdr
        );
        offset += 25;
        ...
        // TODO: Check color_type of IHDR to determine whether a PLTE chunk is
        // expected.
        // let color_type = ihdr.data()[9];

        while offset < len {
            // Length of the chunk data (first 4 bytes of each chunk).
            ...
            // Chunk Size = 4 (len) + 4 (type) + data_len + 4 (CRC)
            let chunk_size = data_len + 12;
...
mod tests {
    ...
    fn chunk_from_parts(chunk_type: &str, data: &[u8]) -> Result<Chunk> {
    ...
    fn generate_chunks() -> Vec<Chunk> {
        // Ensure at minimum the IHDR and IEND chunks are included.
        vec![
            chunk_from_parts(
                "IHDR",
            ...
            .unwrap(),
            chunk_from_parts("FrSt", b"I am the first chunk").unwrap(),
            chunk_from_parts("miDl", b"I am another chunk").unwrap(),
            chunk_from_parts("LASt", b"I am the last chunk").unwrap(),
            chunk_from_parts("IEND", b"").unwrap(),
        ]
    }

    fn generate_png() -> PNG {
        PNG::from_chunks(generate_chunks())
    }
    ...
    fn test_png_valid_from_bytes() {
        let chunk_bytes: Vec<u8> = generate_chunks()
            .into_iter()
            .flat_map(|chunk| chunk.as_bytes())
            .collect();

        let bytes: Vec<u8> = PNG::MAGIC
            .iter()
    ...
    fn test_png_invalid_chunk() {
        let mut chunk_bytes: Vec<u8> = generate_chunks()
            .into_iter()
            .flat_map(|chunk| chunk.as_bytes())
            .collect();
        ...
        let mut bad_chunk = vec![
            0, 0, 0, 5,         // Length
            32, 117, 83, 116,   // Chunk Type (invalid)
            65, 64, 65, 66, 67, // Chunk Data
            1, 2, 3, 4, 5       // CRC (invalid)
        ];
    ...
    fn test_png_append_chunk() {
        ...
        assert_eq!(chunk.to_string(), "Message");
    }
    ...
    // Compressed version of `dice.png` image on Wikipedia.
